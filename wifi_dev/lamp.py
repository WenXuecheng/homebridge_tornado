...
name_to_dev_name = {
    "台灯":"esp-lamp-wifi-dev-table-lamp",
}
...
def option_dev_swtich(name,op,va,ip):
    ...
    try:
        ...
        name=name
    if name == "esp-lamp-wifi-dev-table-lamp":
        if op == 'init':
            if ip != dev_name_and_ip["esp-lamp-wifi-dev-table-lamp"]:
                dev_name_and_ip["esp-lamp-wifi-dev-table-lamp"] = ip
                return 'ip changed'
            return 'init ok'
        if op == 'open':
